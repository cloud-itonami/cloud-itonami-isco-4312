(ns statclerk.actor
  "StatFinanceInsuranceClerksActor — the ISCO-08 4312 community statclerk actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one statclerk operation request
  ...
  The unconditional invariant: the StatFinanceInsuranceClerksAdvisor can never
  directly commit a record the StatFinanceInsuranceClerksGovernor refuses —
...
(defn build-graph
  "Build a compiled StatFinanceInsuranceClerksActor graph. `store` implements
  `statclerk.store/Store`. `advisor` implements
  `statclerk.advisor/Advisor` (defaults to `mock-advisor`).
